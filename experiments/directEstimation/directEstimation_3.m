...
patience = unique(All.patience);
patienceVal     = numel(patience);
succMu   = zeros(patienceVal,1); succStd = zeros(patienceVal,1);
rewMu    = zeros(patienceVal,1); rewStd  = zeros(patienceVal,1);
stepMu   = zeros(patienceVal,1); stepStd = zeros(patienceVal,1);
timeMu   = zeros(patienceVal,1); timeStd = zeros(patienceVal,1);
...
nRuns = arrayfun(@(p) sum(All.patience==p), patience);
alpha = 0.05;
...
fprintf('  Patience   Métrica        Media       IC_lower    IC_upper\n');
fprintf('-------------------------------------------------------------\n');
for i = 1:patienceVal
    p = patience(i);
    % Success rate
...
pLabels = cellstr(string(patience));

% Boxplot de Success rate
figure
boxplot(All.success_rate, All.patience, ...
        'Labels', pLabels, ...
        'Whisker', 1.5)
xlabel('Patience')
ylabel('Success rate')
title('Distribución de Success rate por valor de patience')
grid on

% Boxplot de Recompensa media
figure
boxplot(All.mean_reward, All.patience, ...
        'Labels', pLabels, ...
        'Whisker', 1.5)
xlabel('Patience')
ylabel('Recompensa media')
title('Distribución de Recompensa media por valor de patience')
grid on

% Boxplot de Pasos medios
figure
boxplot(All.mean_steps, All.patience, ...
        'Labels', pLabels, ...
        'Whisker', 1.5)
xlabel('Patience')
ylabel('Pasos medios')
title('Distribución de Pasos medios por valor de patience')
grid on

% Boxplot de Tiempo de entrenamiento
figure
boxplot(All.training_time, All.patience, ...
        'Labels', pLabels, ...
        'Whisker', 1.5)
xlabel('Patience')
ylabel('Tiempo de entrenamiento (s)')
title('Distribución de Tiempo de entrenamiento por valor de patience')
grid on